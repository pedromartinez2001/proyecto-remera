...
const INTEGRATION_VERSION = "pagopar-2026-08-12-v5-package-dimensions";
...
export async function handler(event) {
  ...
  try {
    ...
    const packageData={peso:0.3,largo:30,ancho:25,alto:3};
    const purchaseItems=validated.map(item=>({ciudad:"1",nombre:`${item.name} — ${item.color} / ${item.size}`,cantidad:item.qty,categoria:"980",...packageData,public_key:publicKey,url_imagen:"",descripcion:item.custom?`DTF ${item.custom.printSize}, ${item.custom.side}${item.custom.back?", más espalda":""}${item.custom.text?`, texto: ${item.custom.text}`:""}${item.custom.hasImage?", incluye imagen":""}`:`Remera DTF ${item.color}, talle ${item.size}`,id_producto:item.id,precio_total:item.price*item.qty,vendedor_telefono:"",vendedor_direccion:"",vendedor_direccion_referencia:"",vendedor_direccion_coordenadas:""}));
    if(shipping>0)purchaseItems.push({ciudad:"1",nombre:"Envío nacional",cantidad:1,categoria:"980",...packageData,public_key:publicKey,url_imagen:"",descripcion:`Entrega en ${cityName}`,id_producto:9001,precio_total:shipping,vendedor_telefono:"",vendedor_direccion:"",vendedor_direccion_referencia:"",vendedor_direccion_coordenadas:""});
